...
int main() {
    struct entry head;// 双向链表　头
    list_entry_t* p = &head.node;//头节点
    list_init(p);// 初始化 前后都指向一个
    head.num = 0;// 值初始化
    int i;
    for (i = 1; i != 10; i ++) {
        // 申请一个链表节点的内存单元　地址赋给e
        ...
        e->num = i;//赋值
        list_add(p, &(e->node));//在头p之后添加一个节点e 代入地址
        p = list_next(p);//更新链表头　前移
    }
    //reverse list all node　反向遍历链表
    while ((p = list_prev(p)) != &head.node)
        printf("%d\n", ((struct entry *)p)->num);//打印值
    return 0;
}
#endif
