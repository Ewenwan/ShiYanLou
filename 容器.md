...
### 使用set保存需要忽略统计的单词
      ...
      string word;
      while(cin >> word)
            if(exclude.find(word) == exclude.end())// exclude.find(word) word没有在exclude中出现时，返回 exclude.end()
                  ++word_count[word];// 统计合理的词的出现次数
...
## pair类型 是用来生成特点类型的模板 first 和 second 来访问  map的元素是 pair类型   make_pair(v1, v2)
