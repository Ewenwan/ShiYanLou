...
/*
 * Init and run calls for acro flight mode    特技模式
 */
...
bool Copter::acro_init(bool ignore_checks)
{
   ...
   // set target altitude to zero for reportinga
...
void Copter::acro_run()
{
    ...
    // if not armed set throttle to zero and exit immediately  非解锁
    ...
    // convert the input to the desired body frame rate  角速度
    ...
    // run attitude controller  按 全角速度控制器控制
