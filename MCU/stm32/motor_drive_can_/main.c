...
u8 board_type =1; //0 - 旧板子   1 - 新板子
 ...
 {	 
  ...
  CAN_Mode_Init(CAN_SJW_1tq,CAN_BS2_4tq,CAN_BS1_7tq,12,CAN_Mode_Normal);//CAN初始化正常模式,波特率250Kbps    
	...
	else if(ptForkLift->u8CarID==2) // //右轮  carId    三轮车 为0     四轮车为2
...
/*
	...
	*/
	...
	//四轮车 顶升 25ms控制周期 一圈 45500脉冲
	//ptForkLift->pid_time=25;
	ptForkLift->s16speed_p = 0.4;    // 15  //
	ptForkLift->s16speed_i = 0.08;    // 1  //0.12
	ptForkLift->s16speed_d = 0.08;    // 3     //
	/*
	if(ptForkLift->u8CarID==3){ //抬升电机为原来的三轮车  一圈22000脉冲
	ptForkLift->pid_time=25;
	ptForkLift->s16speed_p = 0.6;    // 15  //
	ptForkLift->s16speed_i = 0.12;    // 1  //0.12
	ptForkLift->s16speed_d = 0.03;      // 3     //
	...
	*/
//	ptForkLift->s16speed_p = 0.2;    // 15  //
//	ptForkLift->s16speed_i = 0.0;    // 1  //0.12
//	ptForkLift->s16speed_d = 0.0;      // 3     //
