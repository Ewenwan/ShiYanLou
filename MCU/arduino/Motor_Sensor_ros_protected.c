...
bool motor_1 = 0;//清洗面6的电机    bit28      1 旋转  0 不转
bool motor_2 = 0;//清洗面5/7的电机    bit29      1 旋转  0 不转
bool motor_3 = 0;//清洗面3/4的电机    bit30      1 旋转  0 不转
bool motor_4 = 0;//清洗面1/2的电机    bit31      1 旋转  0 不转
...
//拉绳参数定义
const int analogInPin = A1;  // 线位移传感器模拟量输出口
...
const int pin = 11;//舵机的控制信号引脚
const int ang_pin = A2;//将舵机的角度输出信号连接至A2
...
// 舵机调换方向判断=========转不动了改变方向==============
  ...
  if(servo_change_dir_dect_time_count >= 500){//1s
    ...
    //Serial2.println(temp);// 打印看看正常情况下 会有多少的 角度差
    // 或者这里 使用 temp的平均值来作为阈值（自适应）
    if(temp>10);// // 可能需要多次都小于一个值才能够判断出 舵机堵转 500ms正常差值 6~7   1s差值 11~20
...
#define DIR_left_front         24 // 左轮方向 IO口
#define DIR_right_front        25 // 右轮方向 IO口
#define DIR_left_behind         22 // 左轮方向 IO口
#define DIR_right_behind        23 // 右轮方向 IO口
...
///*
  ...
  //Serial2.println(liner_speed_B);
 // PWM_A_value = map(liner_speed_A, 0, 0.2, 1000, 15000);
  //PWM_A_value = (int)(liner_speed_A / 0.2 * 15000);
...
else
{
  ...
  PWM_left_value = abs(vel_l) * 254  / 0.2;
  PWM_right_value = abs(vel_r) * 254  / 0.2;
  
 // Serial2.println(PWM_left_value);
  ...
  motor_1 = (cmd_msg.data & 0x10000000)>>28;
  motor_2 = (cmd_msg.data & 0x20000000)>>29;
  motor_3 = (cmd_msg.data & 0x40000000)>>30;
  motor_4 = (cmd_msg.data & 0x80000000)>>31;
  digitalWrite(ledPin, motor_1);

 ////////////A车控制////
 /*
  if(direction_A) digitalWrite(DIR_A, HIGH); // LOW 后退   HIGH向前 
  else  digitalWrite(DIR_A, LOW); // LOW 后退   HIGH向前 
  pwmWriteHR(PWM_A, PWM_A_value); 
*/
//// B车电机方向口/////// 
  // pinMode(DIR_left, OUTPUT);   // 左轮方向口
  if(direction_B)
  {
   if (vel_l >= 0) 
   {
 ...
 //  pinMode(DIR_right, OUTPUT);   // 右轮方向口
   if (vel_r >= 0) 
   {
...
//// B车电机PWM口 //////
  ...
  // SetPinFrequency(PWM_left, Frequence);    //左电机控制频率 初始化
   analogWrite(PWM_left_front,PWM_left_value);            //左电机控制占孔比初始化
   analogWrite(PWM_left_behind,PWM_left_value);            //左电机控制占孔比初始化
   //SetPinFrequency(PWM_right, Frequence);   //右电机控制频率 初始化
   analogWrite(PWM_right_front, PWM_right_value);          //右电机控制占孔比初始化
   analogWrite(PWM_right_behind, PWM_right_value);          //右电机控制占孔比初始化
...
void setup()
{
  ...
  Serial3.begin(9600);//串口初始化  库仑计信号
...
// 舵机初始化=====================================
   // myservo.attach(pin);  // 将舵机的输入连接至9脚
...
//Serial2.println(Infrared_5);
 ...
 while (Serial3.available()) {
    data[count] = (unsigned char)Serial3.read();//读取一个新字节
